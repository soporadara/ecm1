...
export default function Dashboard({ auth, stats, recentOrders }: any) {
    ...
    return (
        <MainLayout title={t('dashboard.title', 'Dashboard')}>
            <Head title={t('dashboard.head', 'My Dashboard')} />
            
            <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-end mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white font-serif">{t('dashboard.heading', 'Customer Dashboard')}</h1>
                        <p className="text-gray-500 mt-2">{t('dashboard.customer_id', 'Customer ID')}: <span className="font-mono font-bold bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">{auth.user.customer_code}</span></p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
                    {[
                        { label: t('dashboard.stats.total', 'Total Orders'), value: stats?.total || 0, icon: '📦', color: 'bg-blue-50 text-blue-600' },
                        { label: t('dashboard.stats.pending', 'Pending Review'), value: stats?.pending || 0, icon: '⏳', color: 'bg-orange-50 text-orange-600' },
                        { label: t('dashboard.stats.delivering', 'Out for Delivery'), value: stats?.delivering || 0, icon: '🚚', color: 'bg-indigo-50 text-indigo-600' },
                        { label: t('dashboard.stats.completed', 'Completed'), value: stats?.completed || 0, icon: '✅', color: 'bg-green-50 text-green-600' },
                    ].map((stat, i) => (
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                    <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t('dashboard.recent_orders', 'Recent Orders')}</h2>
                        <Link href="/my-orders" className="text-brand-primary text-sm font-bold hover:underline">{t('dashboard.view_all', 'View All &gt;')}</Link>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 text-sm">
                                    <th className="p-4 font-medium">{t('dashboard.order_number', 'Order Number')}</th>
                                    <th className="p-4 font-medium">{t('dashboard.date', 'Date')}</th>
                                    <th className="p-4 font-medium">{t('dashboard.items', 'Items')}</th>
                                    <th className="p-4 font-medium">{t('dashboard.status', 'Status')}</th>
                                    <th className="p-4 font-medium text-right">{t('dashboard.action', 'Action')}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {recentOrders && recentOrders.length > 0 ? recentOrders.map((order: any) => (
                                    <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                        ...
                                        <td className="p-4 text-gray-500">
                                            {order.title || `${order.items_count} ${t('dashboard.items_count', 'items')}`}
                                        </td>
                                        <td className="p-4">
                                        ...
                                        <td className="p-4 text-right">
                                            <Link href={`/my-orders/${order.id}`} className="text-brand-primary font-medium hover:underline text-sm">{t('dashboard.view_details', 'View Details')}</Link>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr>
                                        <td colSpan={5} className="p-8 text-center text-gray-500">
                                            {t('dashboard.no_orders', 'No orders found.')} <Link href="/manual-order" className="text-brand-primary hover:underline">{t('dashboard.create_first', 'Create your first order')}</Link>.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
