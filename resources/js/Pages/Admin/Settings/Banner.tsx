...
export default function BannerSettings({ banner }: any) {
    ...
    return (
        <AdminLayout title="Banner Settings">
            <Head title="Banner Settings — Rafel CMS" />
            
            <div className="max-w-4xl">
                <div className="mb-6 flex justify-between items-end">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Hero Banner Settings</h1>
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Configure the main banner on the homepage.</p>
                    </div>
                </div>
                        ...
                        <div className="space-y-6">
                            ...
                            {data.media_source === 'url' ? (
                                <div>
                                    ...
                                    />
                                    {errors.media_url && <p className="text-red-500 dark:text-red-400 text-xs mt-1">{errors.media_url}</p>}
                                </div>
                            ) : (
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-50 dark:file:bg-indigo-900/30 file:text-indigo-700 dark:file:text-indigo-400 hover:file:bg-indigo-100 dark:hover:file:bg-indigo-900/50 transition-colors"
                                    />
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Max size: 50MB. {data.media_type === 'video' ? 'Supports MP4, WEBM.' : 'Supports JPG, PNG, WEBP.'}</p>
                                    {errors.media_file && <p className="text-red-500 dark:text-red-400 text-xs mt-1">{errors.media_file}</p>}
                                </div>
                            )}

                            {/* Media Preview */}
                            <div className="mt-4 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-900 aspect-video relative flex items-center justify-center">
