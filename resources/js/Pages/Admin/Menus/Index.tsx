...
export default function MenusIndex({ menus, pages = [] }: any) {
    ...
    const handleAddSelectedPages = () => {
        ...
        const itemsToAdd = pages
            .filter((p: any) => selectedPages.includes(p.id))
            .map((p: any) => ({
                label: p.title,
                url: `/pages/${p.slug}`,
            }));
        ...
        <AdminLayout title="Menus">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-1/3 space-y-6">
                    ...
                    {selectedMenu && pages.length > 0 && (
                        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
                            <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-4">Available Pages</h3>
                            <div className="max-h-60 overflow-y-auto space-y-2 mb-4 border border-gray-200 dark:border-gray-700 rounded-lg p-2 bg-gray-50 dark:bg-gray-900/50">
                                {pages.map((page: any) => (
                                    <label key={page.id} className="flex items-center space-x-3 p-2 hover:bg-white dark:hover:bg-gray-800 rounded cursor-pointer">
                                        <input 
                                            type="checkbox" 
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                            ...
                            <div className="p-6">
                                        ...
                                        {selectedMenu.items.map((item: any) => (
                                            <div key={item.id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                                                ...
                                                {item.children && item.children.length > 0 && (
                                                    <div className="bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 py-2 pl-8 pr-4 space-y-1">
                                                        {item.children.map((child: any) => (
                                                            <div key={child.id} className="flex justify-between items-center group hover:bg-gray-100 dark:hover:bg-gray-800 p-2 rounded transition-colors">
                                                                <div>
                                                                ...
                                                                <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    <button onClick={() => deleteItem(selectedMenu.id, child.id)} className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 text-xs">Delete</button>
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
                                ...
                                <div className="pt-6 border-t border-gray-200 dark:border-gray-700">
                                    <h4 className="font-medium text-gray-800 dark:text-gray-200 mb-4">Add Custom Link</h4>
                                    <form onSubmit={handleCreateItem} className="grid grid-cols-2 gap-4">
                                        <div className="col-span-2">
                                            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Parent Item (Optional)</label>
                                            <select 
                                            ...
                                            >
                                                ...
                                                {selectedMenu.items?.filter((i: any) => !i.parent_id).map((item: any) => (
