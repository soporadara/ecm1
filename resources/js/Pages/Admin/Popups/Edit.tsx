...
export default function Edit({ popup }: Props) {
    ...
    return (
        <AdminLayout title="Edit Pop-up Ad">
            <Head title={`Edit Pop-up Ad - ${popup.title}`} />
            ...
            <div className="mx-auto max-w-7xl">
                <div className="mb-6 flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-admin-text">Edit Pop-up Ad</h1>
                        <p className="mt-1 text-sm font-semibold text-admin-text-muted">Update the homepage popup content, image, button, and schedule.</p>
                    </div>
                </div>
                    ...
                    <form onSubmit={submit} className="rounded-2xl border border-admin-border bg-admin-surface p-5 shadow-sm">
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="block text-sm font-bold text-admin-text-muted sm:col-span-2">Internal Title
                                <input className={`${inputClass} mt-1.5`} value={data.title} onChange={(event) => setData('title', event.target.value)} required />
                            ...
                            <label className="block text-sm font-bold text-admin-text-muted sm:col-span-2">Public Heading
                                <input className={`${inputClass} mt-1.5`} value={data.heading} onChange={(event) => setData('heading', event.target.value)} />
                            </label>
                            ...
                            <label className="block text-sm font-bold text-admin-text-muted">Button Text
                                <input className={`${inputClass} mt-1.5`} value={data.button_label} onChange={(event) => setData('button_label', event.target.value)} />
                            </label>
                            <label className="block text-sm font-bold text-admin-text-muted">Button Link
                                <input className={`${inputClass} mt-1.5`} value={data.link_url} onChange={(event) => setData('link_url', event.target.value)} />
                            </label>
                            ...
                            <label className="block text-sm font-bold text-admin-text-muted sm:col-span-2">Popup Image
                                <span className="mt-1.5 flex min-h-28 cursor-pointer items-center justify-center rounded-xl border border-dashed border-admin-border bg-admin-surface-muted text-sm font-bold text-admin-text-muted">
                                    ...
                                    Upload replacement image for {creativeSize.dimensions}
